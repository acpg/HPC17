...
#pragma omp parallel shared(a, b, nthreads, locka, lockb) private(tid)
  ...
  #pragma omp sections nowait
    ...
    #pragma omp section
      ...
      omp_set_lock(&locka); // a locked
      for (i=0; i<N; i++)
        a[i] = i * DELTA;
      omp_unset_lock(&locka); //locks need to unset so that the other part can get lock a too
      omp_set_lock(&lockb); // b was already locked, need to wait for b to unlock to continue
    ...
    #pragma omp section
      ...
      for (i=0; i<N; i++)
        b[i] = i * PI;
      omp_unset_lock(&lockb); // locks need to unset to go on
      omp_set_lock(&locka);
