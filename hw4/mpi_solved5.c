...
// Solution: we need to use synchronous blocking in the send, the send waits until matching return
...
{
int        numtasks, rank, i, tag=111, dest=1, source=0, count=0, count2=0;
char       data[MSGSIZE];
...
if (rank == 0) {
  ...
  while (1) {
    ...
    //MPI_ISend(data, MSGSIZE, MPI_BYTE, dest, tag, MPI_COMM_WORLD, &request);
...
if (rank == 1) {
  while (1) {
    ...
    //MPI_IRecv(data, MSGSIZE, MPI_BYTE, source, tag, MPI_COMM_WORLD, &status, &request);
