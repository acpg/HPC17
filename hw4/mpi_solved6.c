...
// Solution: the Waitall was waiting for inexistent requests (inexistent non-blocking operations to finish)
// first of all, since each task is independent, we do not need an offset and 
// second, the number of non-blocking operations is REPS except for rank=2 where it is 0 
